...
export const EditCheckCashier = () => {
    ...
    const getConnector = useCallback(async () => {
        try {
            ...
            setDeletePayment({ ...payment, position: fetch.type })
        } catch (e) {
            notify(e)
        }
    }, [request, connectorId, auth, setConnector, setPayment, setDeletePayment])


    const getSections = useCallback(async () => {
        try {
            const fetch = await request(`/api/section/cashieredit/${connectorId}`, 'GET', null, {
                Authorization: `Bearer ${auth.token}`
            })
            setSections(fetch)
        } catch (e) {
    ...
    const changePaymentSection = (key) => {
        setDeletePayment({ ...deletePayment, total: 0 - parseInt(sections[key].priceCashier), type: payment.type })
    ...
    }, [request, auth, delSection, setDeletePayment, deletePayment])
    ...
    const getPayment = useCallback(async () => {
        try {
            ...
            fetch.map(p => {
                setPayment({
                    ...payment,
                    total: payment.total + p.total,
                    cash: payment.cash + p.cash,
                    card: payment.card + p.card,
                    trandfer: payment.transfer + p.transfer,
                    type: p.type
                })
            })
            setpayments(fetch)
                                ...
                                return (
                                    <tr >
                                        ...
                                        <td style={{ width: "25%", padding: "10px 0" }}>
                                            ...
                                            <input disabled defaultChecked={section.priceCashier === section.price ? true : false} id={`checkbox${key}`} onChange={event => checkboxSection(event, key)} type="checkbox" className="check" style={{ position: "absolute" }} />
                                        </td>
                    ...
                    <div className='row border-top border-bottom p-3'>
                        <div className='col-md-3 col-4 text-center '>
                            <label className='mx-3 ' >
                                <input defaultChecked={payment.type === "card" ? true : false} disabled onChange={setAllPayment} id='card' type="radio" name="payment" /> Plastik
                            </label>
                            <input
                                value={payment.card}
                                type="number"
                        ...
                        <div className='col-md-3 col-4 text-center'>
                            <label className='mx-3'>
                                <input defaultChecked={payment.type === "cash" ? true : false} disabled onChange={setAllPayment} id='cash' type="radio" name="payment" id='cash' /> Naqt
                            </label>
                            <input
                                value={payment.cash}
                                type="number"
                        ...
                        <div className='col-md-3 col-4 text-center'>
                            <label className='mx-3'>
                                <input defaultChecked={payment.type === "transfer" ? true : false} disabled onChange={setAllPayment} id='transfer' type="radio" name="payment" /> O'tkazma
                            </label>
                            <input
