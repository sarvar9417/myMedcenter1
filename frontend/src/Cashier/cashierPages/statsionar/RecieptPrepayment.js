...
toast.configure()
export const RecieptPrepayment = () => {
    ...
    const prepayment = useLocation().state
                                ...
                                <table className="table table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="text-center" style={{ fontSize: "10pt", fontFamily: "times" }}>Xolati</th>
                                            <th className="text-center" style={{ fontSize: "10pt", fontFamily: "times" }}>Oldindan to'lov</th>
                                            <th className="text-center" style={{ fontSize: "10pt", fontFamily: "times" }}>To'lov turi</th>
                                            <th className="text-center" style={{ fontSize: "10pt", fontFamily: "times" }}>To'lov vaqti</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {oldPayments && oldPayments.map(old => {
                                            paid = paid + old.total
                                            return (
                                                <tr>
                                                    <td style={{ fontSize: "10pt", fontFamily: "times" }}>{++k}</td>
                                                    <td style={{ fontSize: "10pt", fontFamily: "times" }} className="ps-3">Statsionar</td>
                                                    <td style={{ fontSize: "10pt", fontFamily: "times" }} className="text-center">{old.total}</td>
                                                    <td style={{ fontSize: "10pt", fontFamily: "times" }} className="text-center">{old.type === "cash"?"naqt": `${old.type==="card"?"plastik":`${old.type === "transfer"?"o'tkazma":"aralash"}`}`}</td>
                                                    <td style={{ fontSize: "10pt", fontFamily: "times" }} className="text-center">
                                                        {new mongoose.Types.ObjectId(old._id).getTimestamp().toLocaleDateString()}<br/>
                                                        {new mongoose.Types.ObjectId(old._id).getTimestamp().toLocaleTimeString()}
                                                    </td>
                                                </tr>
                                            )
                                        })}
